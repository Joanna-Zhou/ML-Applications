...
def stereo_disparity_fast(Il, Ir, bbox, maxd):
    ...
    for row in range(y_1, y_2):
        ...
        print("Size of window_r_list:", len(window_r_list))
        ...
        for col_l in range(x_l1, x_l2):
            ...
            print(len(window_r_list[col_l-x_r1-maxd : col_l-x_r1]))
            for window_r in window_r_list[col_l-x_r1-maxd : col_l-x_r1]:
                SAD_list.append(np.mean(np.absolute(window_l-window_r)))
            print("Size of SAD_list is {} at column {}".format(col_l, len(SAD_list)))
